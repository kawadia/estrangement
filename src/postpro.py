...
#SD: todo: I think only delta is used from here. can simply if so. 
...
def plot_by_param(dictX, dictY, opt,opt_linewidth=2.0,opt_markersize=15,label_fontsize=20,xfigsize=16.0,yfigsize=12.0,fontsize=28, fname=None, listLinestyles=None, xlabel="",
    ...
    for label in sorted(dictX.keys()):
        ...
        if dictErr is not None:
            ...
            line_dict[label] = pyplot.errorbar(
                ...
                fmt=fmt,
                label="%s"%str(label), linewidth=opt_linewidth,
                elinewidth=opt_linewidth / 2.0,
                markersize=opt_markersize)
        else:
...
def plot_temporal_communities(nodes_of_interest=[],deltas_to_plot=[],tiled_figsize='(36,16)',manual_colormap=None,label_cmap='Paired',opt_frameon=True,xlabel="Time",ylabel="Node id",label_fontsize=20,show_title=True,opt_fontsize=28,colorbar=True,show_yticklabels=False,nodelabel_func=None,xtick_separation=5,snapshotlabel_func=None,opt_wspace=0.2,opt_bottom=0.1,image_extension="svg",opt_dpi=200):
    ...
    for delta, runs_iter in itertools.groupby(listOptionsDict,operator.itemgetter('delta')):
        ...
        if prev_ax is not None:
            ax1 = fig1.add_subplot(numRows, numCols, plotNum, sharex=prev_ax,
                sharey=prev_ax, frameon=opt_frameon)
            ax1.get_yaxis().set_visible(False)
        else:    
            ax1 = fig1.add_subplot(numRows, numCols, plotNum, frameon=opt_frameon)
            prev_ax = ax1
        ...
        if show_title is True:
            ax1.set_title("$\delta$=%s"%delta , fontsize=opt_fontsize)
    ...
    fig1.subplots_adjust(wspace=opt_wspace, bottom=opt_bottom)
    
    pylab.savefig('dynconsuper%s.%s'%(suffix,image_extension), dpi=opt_dpi)
    # svg viewers are slow, also save pdf
    pylab.savefig('dynconsuper%s.%s'%(suffix,'pdf'), dpi=opt_dpi)
